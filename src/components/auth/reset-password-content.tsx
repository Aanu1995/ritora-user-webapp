...
type ResetPasswordInnerProps = {
  tokenFromRoute?: string | null;
};

function ResetPasswordInner({
  tokenFromRoute = null,
}: ResetPasswordInnerProps) {
  ...
  const forgotPasswordHref = useMemo(() => AppRoute.ForgotPassword, []);
  ...
  if (!token) {
    return (
      <div className="space-y-4 text-center">
        ...
        <p className="text-sm text-muted">{t('invalidResetTokenHelp')}</p>
        <Button asChild variant="outline" className="w-full rounded-full">
          <Link href={forgotPasswordHref}>{t('requestNewLink')}</Link>
        </Button>
      </div>
    );
  }
...
export function ResetPasswordContent({
  ...
  return (
    <div>
      ...
      >
        <ResetPasswordInner tokenFromRoute={tokenFromRoute} />
      </Suspense>
    </div>
  );
}
