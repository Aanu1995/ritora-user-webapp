    ...
    return (
      <div className="space-y-3">
        ...
        <div className="rounded-2xl border border-border bg-surface-muted p-6 text-center">
          ...
          <p className="mt-3 text-sm font-semibold">{t("noEntryBody")}</p>
          {onAddPhoto ? (
    ...
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-[11px] text-muted">
            {isToday
              ? t("metaToday", { weekday: weekdayShort(date, locale), date })
              : t("meta", { weekday: weekdayShort(date, locale), date })}
          </p>
          <h3 className="font-display text-base font-bold">
            {isToday ? t("todayTitle") : t("title", { date })}
          </h3>
        </div>
        {hasReaction ? (
      ...
      {entry.photo_url ? (
        ...
        <div className="rounded-2xl border border-border bg-surface-muted p-5 text-center">
          ...
          <p className="mt-1 text-xs text-muted">{t("noPhotoBody")}</p>
          {onReplacePhoto ? (
      ...
      {(entry.ratings || entry.complaint_note || entry.overall_feel) && (
        <div className="rounded-2xl border border-border bg-surface p-4 sm:p-5">
          ...
          <div className="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
            {entry.overall_feel ? (
              <div>
                <p className="mb-1 text-xs text-muted">{t("feelLabel")}</p>
            ...
            <div>
              <p className="mb-1 text-xs text-muted">{t("contextLabel")}</p>
              <div className="flex flex-wrap gap-1.5">
                {entry.sleep_band ? (
          ...
          {entry.complaint_note ? (
            <>
              ...
              <p className="mb-1 text-xs text-muted">{t("noteLabel")}</p>
              <p className="m-0 text-xs italic text-foreground">
