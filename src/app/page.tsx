      ...
      <main>
        ...
        <section className="mx-auto grid w-full max-w-7xl items-center gap-12 px-5 pt-10 pb-20 sm:px-6 sm:pt-16 lg:grid-cols-[1.1fr_1fr] lg:gap-16 lg:px-8 lg:pt-24 lg:pb-28">
          <div className="animate-fade-up space-y-8">
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-surface px-4 py-2 text-sm font-medium text-muted shadow-soft">
          ...
          <div className="animate-fade-up-delay-1 relative">
            ...
            <div className="relative">
              <HeroShelfIllustration className="animate-soft-float h-auto w-full" />
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8">
          <div className="animate-fade-up-delay-2 rounded-3xl border border-border bg-surface p-6 shadow-soft sm:p-8">
            ...
            <dl className="mt-4 grid gap-6 sm:grid-cols-3">
              {trustItems.map((item) => (
                <div key={item.label} className="flex items-baseline gap-3">
                  ...
                  <dd className="text-sm leading-6 text-muted">{item.label}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        >
          <div className="max-w-3xl space-y-5">
            ...
            <p className="text-lg leading-8 text-muted">{t("problem.body")}</p>
          </div>
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {problems.map((problem, index) => (
              <article
                key={problem.title}
                className="group animate-fade-up relative overflow-hidden rounded-2xl border border-border bg-surface p-7 shadow-soft transition hover:-translate-y-0.5 hover:border-accent"
              >
                <span
                  aria-hidden="true"
        ...
        >
          <div className="grid gap-14 lg:grid-cols-2 lg:items-center">
            <div className="order-2 lg:order-1">
              <div className="relative">
                <div
                  className="absolute -inset-6 rounded-[3rem] bg-secondary-glow blur-3xl"
                  aria-hidden="true"
                />
              ...
              <ol className="mt-4 space-y-5">
                {steps.map((step, index) => (
                  ...
                  >
                    <span
                      aria-hidden="true"
                      className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-accent-soft text-sm font-semibold text-accent-strong"
                    >
                      {index + 1}
                    </span>
        ...
        >
          <div className="grid gap-14 lg:grid-cols-2 lg:items-center">
            ...
            <div className="relative">
              <div
                className="absolute -inset-6 rounded-[3rem] bg-accent-glow blur-3xl"
                aria-hidden="true"
              />
          ...
          <div className="mt-16 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {features.map((feature) => {
              ...
              return (
                <article
                  key={feature.title}
                  className="group rounded-2xl border border-border bg-surface p-6 shadow-soft transition hover:-translate-y-0.5 hover:border-accent"
                >
                  <span className="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-accent-soft text-accent-strong transition group-hover:scale-105">
        ...
        >
          ...
          <div className="mt-12 space-y-3">
            {faqs.map((faq) => (
              <details
                key={faq.question}
                className="group rounded-2xl border border-border bg-surface p-6 shadow-soft transition open:border-accent"
              >
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-base font-semibold text-foreground">
                  <span>{faq.question}</span>
                  <span
                    aria-hidden="true"
                    className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-border text-muted transition group-open:rotate-45 group-open:border-accent group-open:text-accent"
                  >
                    +
                  </span>
                </summary>
