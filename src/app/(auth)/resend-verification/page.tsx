...
const DEFAULT_RESEND_VERIFICATION_VALUES: ResendVerificationValues = {
  email: '',
};
...
export default function ResendVerificationPage() {
  ...
  const form = useForm({
    defaultValues: {
      ...DEFAULT_RESEND_VERIFICATION_VALUES,
    ...
    validators: {
      onChange: resendVerificationSchema,
      onSubmit: resendVerificationSchema,
      onSubmitAsync: async ({ value }) => {
        const result = await executeMutation(resend.mutate, value.email);
    ...
    <div>
      ...
      {form.state.isSubmitSuccessful ? (
        <div className="mt-8 space-y-4 text-center">
          <p className="text-sm text-accent">{t('verifyEmailSent')}</p>
          <Link
            href={AppRoute.Login}
        ...
        >
          ...
          >
            {({ canSubmit, isSubmitting }) => (
              <Button
                type="submit"
                disabled={!canSubmit || isSubmitting || resend.isPending}
                className="w-full"
              >
