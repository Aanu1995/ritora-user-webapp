...
describe("notifications.service", () => {
  ...
  it("normalizes database time values in notification preferences", async () => {
    ...
    expect(preferences.reaction_alert_channels).toEqual(["email"]);
    expect(preferences.suggestion_ready_channels).toEqual(["in_app"]);
    expect(preferences.product_expiry_alert_channels).toEqual(["in_app"]);
  });

  it("defaults missing notification channels to in-app only", async () => {
    (getRequest as jest.Mock).mockResolvedValue({
      photo_reminder_local_time: "08:00:00",
      photo_reminder_enabled: true,
      reaction_alerts_enabled: true,
    ...
    expect(preferences.channels).toEqual(["in_app"]);
    expect(preferences.suggestion_ready_channels).toEqual(["in_app"]);
    expect(preferences.reaction_alert_channels).toEqual(["in_app"]);
    expect(preferences.insight_alert_channels).toEqual(["in_app"]);
  });
});
